...
void  BankAccount::deposit(double amount) {	// Method to deposit money
	if (amount > 0.0) {
		...
		// Get current time for transaction history -----------------------
		time_t now = time(0); // Get current time 
		std::string timeStr = ctime(&now); // Convert to string  
		timeStr.pop_back(); // Remove newline character
		transactionHistory.emplace_back("Deposit", amount, timeStr); // Add to transaction history
	}
...
void BankAccount::withdraw(double amount) {	// Method to withdraw money
	if (amount <= 0.0) {	// Check for valid withdrawal
		...
		std::string timeStr = ctime(&now); // Convert to string
		timeStr.pop_back(); // Remove newline character
		transactionHistory.emplace_back("Withdrawal", amount, timeStr); // Add to transaction history
	}
}

void BankAccount::printHistory() const {	// Print transaction history -----------------------
	...
	for (const auto& transaction : transactionHistory) {
		std::cout << transaction.timestamp << " - " << transaction.type 
			<< ": $" << transaction.amount << "\n";
	}
}
